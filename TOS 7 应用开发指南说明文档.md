...
| 字段 | 类型 | 必填 | 说明 | 详细描述 |
|---|---|---|---|---|
| `id` | string | ✅ 是 | 应用唯一标识符 | 平台全局唯一，不可与已上架应用重复。字符集：小写字母（`a-z`）、数字（`0-9`）和连字符（`-`）。必须以字母开头。最大长度：64 字符。示例：`my-app-latest`。创建后不可修改。 |
...
1. 访问 TNAS 开发者平台：https://dev.terra-master.com
...
#### 16.2.1 初审阶段
1. 专职审核人员登录应用管理平台（https://app.mgmt.terra-master.com），领取"待初审"任务
