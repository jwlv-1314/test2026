...
> ⚠️ **注意：** Node.js、Java、Go 等语言运行时非 TOS 系统预装，Deb 应用不可直接依赖。详见 [第 2 章 · 应用架构策略](02_应用架构策略.md)。
...
| 模板仓库 | 打包方式 | 适用场景 |
|---|---|---|
| [Deb 应用模板（单包）](https://github.com/terra-master/app-template-deb) | 单包模式 | 从零开发的新应用，所有文件统一打包 |
| [Deb 应用模板（双包）](https://github.com/terra-master/app-template-deb-dual) | 双包模式 | 已有通用标准 deb 包的应用 |
| [Docker 应用模板](https://github.com/terra-master/app-template-docker) | Docker | Docker 容器化部署 |

> ⚠️ **注意：** 子类型（WebUI 内部/外部/无 UI）与打包方式（单包/双包）是两个独立维度，可交叉组合。单包和双包均支持三种子类型。
