...
**Deb 应用**使用以下 GitHub Actions 工作流。将 `<appid>` 替换为你的实际应用 ID，将 `./app-root` 替换为你的 Deb 包根目录路径。
...
添加校验工作流，每次推送时检查配置文件。校验脚本还应检查 `type` 与 `open_path` 是否同时存在，若同时存在则报错。
    ...
    steps:
      ...
      - name: 校验必填字段
        run: |
          ...
          # 校验 type 与 open_path 不能同时存在
          if 'type' in config and 'open_path' in config:
              raise AssertionError('type 与 open_path 不能同时存在，只能选择一个')
