...
1. **Option A: Ubuntu 22.04 Virtual Machine (Recommended)**
   ...
   - VM includes pre-configured TOS7 tools and simulated services
...
3. **Option C: Physical TNAS Device (for Final Testing)**
   - Final verification must be performed on actual hardware before submission
...
# 5. Check Web UI accessibility (Web apps)
...
# 7. Test uninstall
sudo dpkg --remove <appid>       # Preserve configuration
sudo dpkg --purge <appid>        # Complete removal

# 9. Verify cleanup (no leftover files/services)
...
|---|---|---|
| Service fails to start | Missing dependencies or incorrect paths | Check `journalctl -u <appid>`, verify `ExecStart` path |
...
| Web UI inaccessible | Service not listening or firewall issue | Check if service is running, verify port binding (`0.0.0.0` not `127.0.0.1`) |
...
| App behaves abnormally after TOS update | ABI changes or service conflict | Check `low_version`, test on new TOS version |
| Configuration not loaded | Incorrect config path or permissions | Verify WorkingDirectory and config file paths |
| Config permissions lost after upgrade | chown/chmod not reapplied in postinst | Add `chown -R <appid>:<appid>` in postinst script |
| Startup fails due to leftover socket file | Socket not cleaned from previous run | Add `rm -f /var/api/<appid>.sock` before starting service |
| Nginx reload fails | Invalid Nginx config syntax | Validate with `nginx -t` before reloading |
| Docker volume permissions incorrect | Host/container UID/GID mismatch | Use `PUID`/`PGID` env vars matching host user |
...
| Deb fails to install due to unmet dependencies | Missing Depends in DEBIAN/control | Declare all required packages in the `Depends` field |

---

← [Previous: Best Practices](12_Best_Practices.md) &nbsp;&nbsp;|&nbsp;&nbsp; [Next: CICD Guide](14_CICD_Guide.md) → &nbsp;&nbsp;|&nbsp;&nbsp; [📖 Return to Contents](../README.md)
